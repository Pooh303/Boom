...
function RejoinView({}) {
  ...
  function rejoin() {
    const id =
      document?.getElementById("id-input")?.value?.toUpperCase() || null;
  ...
  return !manualInput ? (
    ...
    <div>
      <h1 className="text-title w-full text-center p-4 text-primary text-2xl font-extrabold">
        KABOOM
      </h1>
      <div className="w-full p-4 bg-neutral flex flex-col items-center gap-4 text-title">
        <h1 className="text-xl text-neutral-content font-extrabold">
          3 letter player id
        </h1>
        <input
          autoComplete="off"
        ...
        <button
          id="join_btn"
          className={"btn btn-wide transition-all btn-secondary "}
          onClick={() => rejoin()}
        >
          Rejoin room
        </button>
      ...
      <a
        className="link font-bold clickable w-full flex items-center justify-center mt-2 -mb-2"
        href="/"
      >
        Leave
      </a>
      ...
      <div className="p-4 rounded-lg m-4 bg-info/30 text-info-content bold-child-info">
        Your <b>3 LETTER ID</b> can be found by pressing the{" "}
        <b className="inline-block">
          <TbCards />
        </b>{" "}
        <b>button</b> on your friends screen. <br />
        (The ID will be next to your username)
      </div>
    </div>
  );
}
