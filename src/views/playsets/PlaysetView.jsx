...
import MegaButton, {
  BigAbsoluteMakeButton,
  BookmarkMegaButton,
  DeletePlaysetButton,
  EditPlaysetButton,
  RemixButton,
} from "../../components/MegaButtons";
...
function PlaysetView({}) {
  ...
  const {
    ...
    queryKey: ["playset", id, user?.id],
    queryFn: fetchPlayset,
  ...
  const deletePlayset = useCallback(() => {
    setPrompt({
      ...
      onApprove: async () => {
        ...
        const { error } = await supabase.from("playsets").delete().eq("id", id);
        if (error) {
          promiseReject(error);
        } else {
          promiseResolve();
  ...
  const _isPresetPlayset = useMemo(
    () => isPresetPlayset(id) || isPresetPlayset(playset?.id),
    [id, playset],
  );
  ...
  const handleBookmark = useCallback(
    async (mark) => {
      ...
      if (_isPresetPlayset) return; // preset playsets can't be bookmarked
    ...
    [id, user, _isPresetPlayset],
  );

  const createRoom = useCallback(async (playset_id) => {
    setPrompt({
      element: (
        <NamePrompt onEnter={setNameAndCreate} buttonValue="CREATE GAME" />
      ),
    });
  ...
  const handleOfficialize = useCallback(
    async (mark) => {
      if (!user)
        return toast.error("You need to be an admin to make this official");
  ...
  const handleDev = useCallback(
    async (mark) => {
      if (!user)
        return toast.error(
          "You need to be an admin to make this a dev playset",
        );
  ...
  return (
    <div className="flex flex-col lg:flex-row w-full h-full overflow-x-hidden overflow-y-scroll scrollbar-hide pb-64">
      ...
      <div className="w-full lg:max-w-3xl flex flex-col items-center justify-start border-neutral/10 lg:border-r">
        {" "}
        ...
        <div className="w-full max-w-2xl p-4 flex flex-col items-center">
          ...
          {playsetMaximized && (
            <PlaysetDisplay
              key={user?.id + "'s playset"}
              autoFetchInteractions
              quickActions={{ vote: true, profile: true }}
              forceOpen
              playset={playsetMaximized}
            />
          )}
          ...
          <div className="w-full flex flex-col gap-2 mt-2">
            <div
              className={
                "w-full grid gap-2 " +
                (_isPresetPlayset ? " grid-cols-1 " : " grid-cols-2 ")
              }
            >
              <RemixButton
                onClick={() =>
                  checkAuth(() =>
                    smoothNavigate(`/workbench/${playset?.id}/remix`),
                  )
                }
              />
              {!_isPresetPlayset && (
                <BookmarkMegaButton
                  ...
                  onChange={(...arr) => checkAuth(() => handleBookmark(...arr))}
                />
              )}
            </div>
            {playset?.user_id === user?.id && (
              <div className="w-full grid grid-cols-2 gap-2">
                <EditPlaysetButton
                  onClick={() =>
                    checkAuth(() =>
                      smoothNavigate(`/workbench/${playset?.id}/edit`),
                    )
                  }
                />
                <DeletePlaysetButton
                  onClick={() => checkAuth(() => deletePlayset())}
                />
              </div>
            )}
            <MegaButton
        ...
        {hasPermission("playset_mod") && (
          <>
            ...
            <div className="w-full flex justify-between items-center p-4 gap-2 max-w-2xl">
              ...
              <MegaButton
                color={"#000000" + (official ? "" : "60")}
                fill
                className={"text-xl"}
                onClick={() => handleOfficialize(!official)}
                showDot={official}
              >
              ...
              <MegaButton
                ...
                className={
                  "text-xl " +
                  (dev ? " dev-mode-stripes " : " bg-amber-400/60! ")
                }
                onClick={() => handleDev(!dev)}
                showDot={dev}
              >
                <MdConstruction />
              </MegaButton>
              ...
              <MegaButton
                color={"#0bcae3" + (ghost ? "" : "60")}
                fill
                className={"text-xl"}
                onClick={() => handleGhost(!ghost)}
                showDot={ghost}
              >
                <FaGhost />
              </MegaButton>
            ...
            {ghost && (
              <div className="w-full max-w-2xl -mt-2 px-4 flex flex-col">
                {!editHiddenReason && (
                  <>
                    {hiddenReason?.length > 2 ? (
                      <>
                        ...
                        />
                        <p
                          className="font-bold text-info hover:underline text-sm flex items-center gap-1"
                          onClick={() => setEditHiddenReason(true)}
                        >
                          Edit <FaPen size={10} />
                        </p>
                      </>
                      ...
                      <div
                        ...
                        onClick={() => setEditHiddenReason(true)}
                      >
                        Why did you hide this playset?{" "}
                        <span className="font-bold text-info group-hover:underline">
                          Add a reason!
                        </span>
                      </div>
                    )}
                ...
                {editHiddenReason && (
                  ...
                  >
                    <textarea
                      autoFocus
                      id="hidden-reason-admin-text"
                      defaultValue={hiddenReason || ""}
                      className="border-2 border-neutral rounded-xl p-2 text-sm"
                    />
                    <div className="w-full flex items-center gap-2 justify-between text-normal mt-2">
                      <button
                        className="btn btn-ghost noskew btn-sm"
                        onClick={() => handleReason(null)}
                      >
                        <FaTrash className="skew text-error" />
                      </button>

                      <div className="flex">
                        <button
                          onClick={() => setEditHiddenReason(false)}
                          className="btn btn-ghost noskew btn-sm"
                        >
                          Cancel
                        </button>
                        <button
                          type="submit"
                          className="btn btn-secondary noskew btn-sm"
                        >
                          save
                        </button>
                      </div>
...
function OfficialPlaysetBanner() {
  return (
    <InfoBanner className={"bg-black pl-3"} size="sm">
      <BsFillCheckSquareFill size={20} className="mr-3" /> Official Kaboom
      playset
    </InfoBanner>
  );
}
...
function GhostPlaysetBanner({ onClick }) {
  return (
    <InfoBanner
      onClick={onClick}
      size="sm"
      style={{ backgroundColor: "#0bcae3" }}
      className={"pl-3 pr-2 text-normal"}
      endElement={
        <button
          style={{ color: "#0bcae3" }}
          className="btn btn-xs  bg-white hover:bg-white border-none noskew"
        >
          reason
        </button>
      }
    >
      ...
      <div className="flex flex-col justify-center tracking-tighter ml-3">
        <h3 className="font-extrabold text-base text-normal">
          Your playset was hidden
        </h3>
        <p className="text-xs text-normal -mt-1">
          It may have violated the{" "}
