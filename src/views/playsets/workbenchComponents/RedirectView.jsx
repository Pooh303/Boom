...
export default function WorkbenchRedirectView({
  editMode = false,
  remixMode = false,
}) {
  ...
  useEffect(() => {
    setTimeout(async () => {
      ...
      setPlayset(
        maximizePlayset(
          await getPlaysetById(playsetId, null, { ignoreCache: true }),
        ),
      );
      setLoading(false);
    }, 2000);
  ...
  return loading ? (
    ...
    <RedirectOptionsView playset={playset} text={"You made this playset:"}>
      <RemixButton
        onClick={() => smoothNavigate(`/workbench/${playset.id}/remix`)}
      />
      <EditPlaysetButton
        onClick={() => smoothNavigate(`/workbench/${playset.id}/edit`)}
      />
    </RedirectOptionsView>
  ) : (
    <RedirectOptionsView
      playset={playset}
      text={"Someone else made this playset:"}
    >
      <RemixButton
        onClick={() => smoothNavigate(`/workbench/${playset.id}/remix`)}
      />
      <MegaButton
        ...
        fill
        color={"#72c4ff"}
        onClick={() => smoothNavigate(`/playsets/${playset.id}`)}
      >
  ...
  return (
    <div className=" w-full h-full flex flex-col items-center justify-center  transition-all">
      ...
      <div className="  animate__animated animate__fadeIn w-full h-full p-4 flex flex-col max-w-lg items-center justify-center  transition-all gap-1">
        <h1 className="-mt-28 text-7xl font-extrabold text-normal text-center">
          404
        </h1>
        ...
        <div className="flex w-full items-center justify-center gap-2">
          ...
          <MegaButton
            title="Make playset"
            Icon={<FaPlus className="group-hover:-rotate-180 transition-all" />}
            fill
            color={"#fc021b"}
            onClick={() => smoothNavigate("/workbench")}
          >
            Playset
          </MegaButton>
        </div>
...
export function RedirectOptionsView({ children, text, playset }) {
  return (
    <div className=" w-full h-full flex flex-col items-center justify-center  transition-all">
      <TitleBar />
      <div className="  animate__animated animate__fadeIn w-full h-full p-4 flex flex-col max-w-lg items-center justify-center  transition-all gap-1">
        <h1 className="-mt-28 text-2xl font-extrabold text-title text-center">
          {text}
        </h1>
