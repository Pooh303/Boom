...
import {
  ...
  getCardFromId,
  getCardsForPlayset,
  getLinkedCardsPaired,
  getLinkedCardsPairedById,
  pairUpCards,
} from "../../helpers/cards";
import {
  avgFromCards,
  getDifficultyDataFromValue,
...
import RedirectView, {
  RedirectLoadingView,
} from "./workbenchComponents/RedirectView";
...
export default function WorkbenchView({
  ...
  const [primaries, setPrimaries] = useState(
    getStartingValues("primaries", startingPlayset),
  );
  const [generalCards, setGeneralCards] = useState(
    getStartingValues("generalCards", startingPlayset),
  );
  const [oddCard, setOddCard] = useState(
    getStartingValues("oddCard", startingPlayset),
  ); // nullable
  const [defaultCards, setDefaultCards] = useState(
    getStartingValues("defaultCards", startingPlayset),
  );
    ...
    switch (type) {
      case "primaries":
        if (startingPlayset?.primaries) {
          return pairUpCards(startingPlayset?.primaries)?.map((cardPair) =>
            cardPair?.map((c) => c?.id),
          );
        } else {
          return [["b001", "r001"]];
      ...
      case "generalCards":
        if (startingPlayset?.cards) {
          return pairUpCards(startingPlayset?.cards)?.map((cardPair) =>
            cardPair?.map((c) => c?.id),
          );
        } else {
          return [["b014", "r014"]];
      ...
      case "defaultCards":
        if (startingPlayset?.default_cards) {
          return pairUpCards(startingPlayset?.default_cards)?.map((cardPair) =>
            cardPair?.map((c) => c?.id),
          );
        } else {
          return [["b000", "r000"]];
  ...
  const [description, setDescription] = useState(
    startingPlayset?.description || "",
  );
  const [shuffle, setShuffle] = useState(
    startingPlayset ? startingPlayset?.shuffle : true,
  );
  const [minPlayers, setMinPlayers] = useState(
    startingPlayset?.min_players || 6,
  );
  const [maxPlayers, setMaxPlayers] = useState(
    startingPlayset?.max_players || 30,
  );
  const [buryOption, setBuryOption] = useState(
    startingPlayset
      ? startingPlayset?.no_bury
        ? "never"
        : startingPlayset?.force_bury
          ? "always"
          : "auto"
      : "auto",
  );
  ...
  const playset = useMemo(() => {
    const playset = {
      ...
      default_cards: crackOpenPairs(defaultCards).map((cid) =>
        getCardFromId(cid),
      ),
      ...
      difficulty: getDifficultyDataFromValue(
        avgFromCards(allCardsInPlaysetInRowId.map((cid) => getCardFromId(cid))),
      )?.difficulty,
    };
    if (remixMode) {
  ...
  // functions
  function crackOpenPairs(pairs) {
    // makes 1d array from 2d array
    var cracked = [];
  ...
  const publishPlayset = useCallback(async () => {
    ...
    if (
      remixMode &&
      startingPlayset?.name?.toLowerCase() === playset?.name.toLowerCase()
    ) {
    ...
    // Remove fields that don't exist as columns in the Supabase "playsets" table
    const { players, ...playsetPayload } = {
      ...playsetCopy,
      min_players: minPlayers,
      max_players: maxPlayers,
    };

    console.log(playsetPayload);

    const { data, error } = await supabase
      .from("playsets")
      .upsert([playsetPayload])
      .select()
      .single();
  ...
  function onPrimaryCardsClick(clickedIndex) {
    var primaryCards = getAllCards()?.filter(
      (card) => card?.primary && !allCardsInPlaysetInRowId.includes(card?.id),
    );
    setPageCover({
      ...
      element: (
        <CardsFilter
          paired
          showDifficulty
          onClick={(card) => promptCardInfoBeforeSelect(card, replaceOrAddCard)}
          filter={{ visibleCards: primaryCards.map((c) => c?.id) }}
        />
      ),
  ...
  function onGeneralCardsClick(clickedIndex) {
    ...
    setPageCover({
      title: t("general_cards"),
      element: (
        <CardsFilter
          paired
          showDifficulty
          onClick={(card) => promptCardInfoBeforeSelect(card, replaceOrAddCard)}
          filter={{ visibleCards: applicableCards.map((c) => c?.id) }}
        />
      ),
      onClose: () => setPageCover(null),
    });
  ...
  function onOddCardClick() {
    var applicableCards = getAllCards()?.filter(
      (card) =>
        !card?.primary &&
        card?.links?.length <= 0 &&
        !["red", "blue"].includes(card?.color_name),
    );
    setPageCover({
      title: t("odd_card"),
      element: (
  ...
  function onDefaultCardsClick(clickedIndex) {
    var applicableCards = getAllCards()?.filter(
      (card) =>
        !card?.primary &&
        (["red", "blue"].includes(card?.color_name) ||
          (card?.color_name === "yellow" && card?.links?.length === 1) ||
          card?.id === "y000"),
    );
    setPageCover({
      title: t("default_cards"),
      element: (
        <CardsFilter
          paired
          showDifficulty
  ...
  function promptCardInfoBeforeSelect(card, replaceOrAddCard = () => {}) {
    setMenu(
      <CardInfoMenu
        card={card}
        color={card?.color}
        onSelect={(card) => handleSelect(card)}
      />,
    );
  ...
  return (
    <div className="flex flex-col md:flex-row w-full h-full overflow-x-hidden overflow-y-scroll scrollbar-hide ">
      <div className="w-full md:max-w-md xl:max-w-xl flex flex-col items-start justify-start grow border-neutral/10 border-b md:border-r">
        {" "}
        {/* Left Bar With linked cards box */}
        ...
        <div className="w-full md:overflow-x-hidden md:overflow-y-scroll gap-4 p-4 md:pb-20 flex flex-col">
          ...
          >
            {primaries.map((cardsIds, i) => (
              <WorkbenchLinkedCards
                onX={() =>
                  setPrimaries((primaries) => removeAtIndex(primaries, i))
                }
                onClick={() => onPrimaryCardsClick(i)}
                onInfo={onCardSetInfo}
                key={"primary-" + i + cardsIds?.[0]?.id}
                id={cardsIds[0]}
              />
            ))}
          </WorkbenchPlaysetArea>
          ...
          >
            {generalCards.map((cardsIds, i) => (
              <WorkbenchLinkedCards
                onX={() =>
                  setGeneralCards((generalCards) =>
                    removeAtIndex(generalCards, i),
                  )
                }
          ...
          >
            {defaultCards.map((cardsIds, i) => (
              <WorkbenchLinkedCards
                onX={() =>
                  setDefaultCards((defaultCards) =>
                    removeAtIndex(defaultCards, i),
                  )
                }
      ...
      <div className="p-4 grow md:overflow-x-hidden md:overflow-y-scroll scrollbar-hide gap-4 flex flex-col items-center">
        ...
        <div className="w-full text-left">
          {t("cards_in_game")} {allCardsInPlaysetInRowId?.length || 0}
        </div>
        <div className="flex flex-col w-full items-center gap-2 ">
          <div className="w-full flex items-center gap-2 text-center">
            <div className="dropdown input border-2 p-0 rounded-md z-100">
              <div
                tabIndex={0}
                role="button"
                className="w-16 h-full flex items-center justify-center cursor-pointer select-none"
              >
                {playset?.emoji}
              </div>
              <div
                tabIndex={0}
                className="dropdown-content z-100 mt-1 shadow-2xl rounded-2xl bg-base-100"
              >
                <Picker
                  onEmojiClick={(data) => {
                    setEmoji(data?.emoji || "🎲");
            ...
            <input
              ...
              defaultValue={name}
              className="input border-2 w-full rounded-md"
          ...
          <div className="w-full -mb-1.5">
            <textarea
              ...
              className="textarea w-full border-2 border-neutral "
              rows="3"
          ...
          <div className="w-full">
            <ToggleButton
              full
              checked={!shuffle}
              onChange={() => setShuffle((shuffle) => !shuffle)}
              recommended={false}
            >
              {t("distribute_in_order")} <Info tooltip="Click me!" href={""} />
            </ToggleButton>
          </div>
          <div className="flex items-center justify-between w-full">
            <h3 className="font-bold">{t("player_range")}</h3>
            <div className="flex items-center gap-2">
              ...
              <RangeCounter
                ...
                min={minPlayers <= 6 ? 6 : minPlayers}
                max={1000}
              />
            </div>
          </div>
          <div className="flex items-center justify-between w-full">
            <h3 className="font-bold">{t("burying")}</h3>
            <select
              value={buryOption}
              onChange={(e) => setBuryOption(e?.target?.value || "auto")}
              className="select select-bordered w-fit border-2 border-neutral"
            >
              <option value="auto">{t("auto")}</option>
              <option value="always">{t("always_bury")}</option>
              <option value="never">{t("never_bury")}</option>
            </select>
          </div>
          <button
            style={{
            ...
            className="btn btn-secondary border-none w-full text-title text-base-100 noskew"
            onClick={() => publishPlayset()}
          >
            {loading ? (
              <div className="loading loading-spinner text-white" />
            ) : remixMode ? (
              t("save_remix")
            ) : (
              t("save_playset")
        ...
        {devMode && false && (
          <textarea
            rows={10}
            value={JSON.stringify(playset)}
            className="textarea w-full border-2 border-neural"
          />
        )}
        <div className="py-12" />
      </div>
...
export function PlaysetSimulator({ playset, buryOption = "auto" }) {
  ...
  const { cards, soberCard } = useMemo(() => {
    ...
    const cardsForPlayset = getCardsForPlayset({
      maximizedPlayset: playset,
      playerCount,
      playWithBury,
    });
  ...
  const cardsPlusSober = useMemo(
    () => [...cards, soberCard]?.filter((c) => c),
    [cards, soberCard],
  );
  ...
  const recommendBury = useMemo(() => {
    return playset?.odd_card && playset?.odd_card?.id !== "drunk"
      ? false
  ...
  useEffect(() => {
    if (playset?.force_bury) {
      setPlayWithBury(true);
    } else if (playset?.no_bury) {
      setPlayWithBury(false);
    } else {
      setPlayWithBury(recommendBury);
    }
  }, [recommendBury, playset?.force_bury, playset?.no_bury]);
  ...
  return (
    <div className="w-full flex flex-col items-center gap-4">
      <div className="font-extrabold w-full text-lg mt-4 text-center border-b-2 border-neutral/30 pb-2 relative">
        <p>{t("simulate_playset")}</p>
        <div className=" absolute top-0 right-0 bottom-0 flex items-center">
          <button
            onClick={() => reload()}
            className={
              "clickable rounded-full w-9 h-9 flex items-center justify-center " +
              (reloading ? " animate-spin " : " animate-none ")
            }
          >
            <TfiReload style={{ transform: "scaleY(-1)" }} size={20} />
          </button>
        </div>
      </div>
      <div className="flex flex-col items-center w-full gap-2">
        ...
        <div className="flex items-center w-full justify-between gap-2 text-lg">
          <ToggleButton
            disabled={
              buryOption !== "auto" || playset?.force_bury || playset?.no_bury
            }
            full
            checked={playWithBury}
            onChange={() => setPlayWithBury((playWithBury) => !playWithBury)}
      ...
      <div className="w-full flex-wrap flex border-2 border-neutral rounded-xl gap-2 p-2">
        {playset?.primaries?.map((card, i) => (
          <ShuffledInCardDummy
            disabled={!cardsPlusSober?.includes(card?.id)}
            key={card?.id + i}
            card={card}
            areaId="primaries"
          />
        ))}
        {playset?.cards?.map((card, i) => (
          <ShuffledInCardDummy
            disabled={!cardsPlusSober?.includes(card?.id)}
            key={card?.id + i}
            card={card}
        ...
        {playset?.default_cards?.map((card, i) => (
          <ShuffledInCardDummy
            disabled={
              cardsPlusSober?.filter((c) => card?.id === c)?.length -
                playset?.cards?.filter((cs) => cs?.id === card?.id)?.length <
              1
            }
            key={card?.id + i}
            card={card}
          ...
          >
            {cardsPlusSober?.filter((c) => card?.id === c)?.length -
              playset?.cards?.filter((cs) => (cs?.id || cs) === card?.id)
                ?.length >
              1 && (
              <ActionCircle
                inverted
                icon={
                  <p className="font-extrabold">
                    {cards?.filter((c) => card?.id === c)?.length -
                      playset?.cards?.filter(
                        (cs) => (cs?.id || cs) === card?.id,
                      )?.length}
                  </p>
                }
        ...
        {playset?.odd_card && (
          <ShuffledInCardDummy
            disabled={!cardsPlusSober?.includes(playset?.odd_card?.id)}
            key={playset?.odd_card?.id}
            card={playset?.odd_card}
            areaId="odd"
          />
        )}
      </div>
      {devMode && (
        <div className="w-full flex flex-wrap gap-2 px-2.5 pb-4">
          {cardsPlusSober?.map((card, i) => (
            <ShuffledInCardDummy key={i} card={getCardFromId(card)} />
          ))}
        </div>
      )}

      <p className="w-full text-sm -mt-4">
        {t("cards_in_game")} {cards?.length}
      </p>
      <div className="grid grid-cols-2 gap-2 w-full">
        {buriedCard && (
          <div className="flex flex-col w-full items-center justify-center gap-2 text-lg font-bold">
            <h3 className="truncate">{t("buried_card_workbench")}</h3>
            <div className="card relative scale-[25%] -mx-24  -my-36 ">
              <CardFront
                card={getCardFromId(buriedCard)}
                color={getCardFromId(buriedCard)?.color}
              />
            </div>
          </div>
        )}
        {soberCard && (
          <div className="flex flex-col w-full items-center justify-center gap-2 text-lg font-bold">
            <h3 className="truncate">{t("sober_card")}</h3>
            <div className="card relative scale-[25%] -mx-24  -my-36 ">
              <CardFront
                card={getCardFromId(soberCard)}
                color={getCardFromId(soberCard)?.color}
              />
            </div>
          </div>
        )}
...
export function ShuffledInCardDummy({
  card,
  areaId,
  disabled = false,
  children,
  ...
  return (
    <div
      className={
        "bg-black rounded-md overflow-hidden w-fit h-fit transition-all group " +
        (disabled ? " scale-75 " : " scale-100 ")
      }
    >
      <div
        style={{ backgroundColor: card?.color?.primary }}
        className={
          "text-white flex items-center justify-center flex-col relative aspect-2/3 w-10 transition-all " +
      ...
      >
        ...
        {card?.src && card?.src !== "" && (
          <img
            src={`/cards${card.src}`}
            className="opacity-100 group-hover:opacity-0 transition-all absolute inset-0"
          />
        )}
...
export function WorkbenchLinkedCards({
  id,
  onInfo = (card) => {},
  onClick = () => {},
  onX = () => {},
...
export function ActionCircle({
  ...
  return (
    <div
      onClick={onClick}
      className={
        " rounded-full  flex items-center justify-center  clickable " +
        (tooltip && " tooltip ") +
        (hidden ? " h-0 w-0 my-0 " : " h-8 w-8 my-1 ") +
        (inverted
...
export function TitleBar({ titleElement, fixed }) {
  ...
  return (
    <div
      className={
        "flex items-center h-14 p-4 z-10 relative justify-between w-full text-xl md:text-2xl font-extrabold text-title transition-all " +
    ...
    >
      <div className="flex items-center h-full">
        <button
          ...
          className="flex items-center justify-end text-primary mr-4 h-full cursor-pointer"
        >
...
export function VerticalDivider() {
  return <div className="h-full p-px rounded-full bg-neutral/20" />;
}
