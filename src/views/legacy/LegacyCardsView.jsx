...
function LegacyCardsView({}) {
  ...
  useEffect(() => {
    ...
    setSearch(initParam || "");
    if (initParam && window?.history?.pushState)
      window?.history?.pushState({}, "", `/legacy/cards`);
  }, []);
  ...
  useEffect(() => {
    if (focusedCard) {
      setTimeout(() => {
        setMenu(<CardInfoMenu card={focusedCard} color={focusedCard.color} />);
  ...
  function sortAllCards() {
    ...
    allSorted = allSorted.sort((x, y) => {
      return x.id.startsWith("y") ? 1 : y.id.startsWith("y") ? -1 : 0;
    });
    allSorted = allSorted.sort((x, y) => {
      return x.id.startsWith("g") ? 1 : y.id.startsWith("g") ? -1 : 0;
    });
    allSorted = allSorted.sort((x, y) => {
      return x.id.startsWith("p") ? 1 : y.id.startsWith("p") ? -1 : 0;
    });
    allSorted = allSorted.sort((x, y) => {
      return x.id.startsWith("e") ? 1 : y.id.startsWith("e") ? -1 : 0;
    });
    allSorted = allSorted.sort((x, y) => {
      return x.id.startsWith("s") ? 1 : y.id.startsWith("s") ? -1 : 0;
    });
  ...
  return (
    <div className="flex flex-col justify-start items-center w-full h-full  overflow-x-hidden">
      ...
      <a
        href="/"
        className="text-title flex items-center justify-center gap-4 text-3xl mt-10 my-6 font-extrabold"
      >
      ...
      <div className="flex flex-wrap items-start justify-center gap-4 p-4 w-full overflow-y-scroll scrollbar-hide h-fit pt-0 pb-24 ">
        {visibleCards.map((card) => (
          <div
            key={card.id}
            onClick={() => navigate(`/legacy/cards/${card.id}`)}
            className="relative scale-[50%] overflow-hidden w-fit -mx-16 -m-24"
          >
            <>
              <CardFront key={card.id} card={card} color={card?.color} />
              {devMode ? (
                <div className="text-2xl font-extrabold">{card.id}</div>
              ) : (
                ""
              )}
            </>
  ...
  return (
    <div className="bg-base-100 m-3 w-full max-w-xl rounded-lg flex justify-between items-center px-2 ">
      <input
        ...
        className="p-1.5 w-full bg-base-200  rounded-lg text-sm px-3 font-semibold border-none outline-none overflow-x-hidden"
      />
      {value.length > 0 && (
        <div onClick={() => onChange("")} className="px-2">
          <IoClose color={"#656565"} size="20" />
        </div>
      )}
    </div>
  );
