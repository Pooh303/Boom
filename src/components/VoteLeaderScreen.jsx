...
function VoteLeaderScreen({ players = [], me, votes = {}, execute, isHost, leaders = {}, totalPlayerCount = 0 }) {
    ...
    const voteCounts = useMemo(() => {
        const counts = {};
        roomPlayers.forEach(p => { counts[p.id] = 0 });
    ...
    return (
        <div className='absolute inset-0 flex flex-col items-center justify-start text-title text-white drop-shadow-md gap-0 text-center overflow-y-scroll overflow-hidden scrollbar-hide z-[95]'
            style={{ background: 'linear-gradient(145deg, #1a0533 0%, #0f0a2e 30%, #1b0c3a 60%, #2d1150 100%)' }}
        >
            {/* Floating orbs background */}
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
                <div className="absolute w-64 h-64 rounded-full opacity-10 animate-pulse" style={{ background: 'radial-gradient(circle, #8b5cf6, transparent)', top: '-5%', left: '-10%' }} />
                <div className="absolute w-80 h-80 rounded-full opacity-10 animate-pulse" style={{ background: 'radial-gradient(circle, #a855f7, transparent)', bottom: '-10%', right: '-15%', animationDelay: '1s' }} />
                <div className="absolute w-40 h-40 rounded-full opacity-5 animate-pulse" style={{ background: 'radial-gradient(circle, #c084fc, transparent)', top: '40%', left: '60%', animationDelay: '2s' }} />
            </div>
            ...
            <div className='relative z-10 flex flex-col items-center pt-8 pb-1 w-full'>
                <div style={{ animationDelay: '0ms' }} className='animate__animated animate__bounceInDown flex items-center justify-center gap-3 mb-1'>
                    <FaCrown className="text-amber-400 text-3xl drop-shadow-lg" style={{ filter: 'drop-shadow(0 0 10px rgba(251, 191, 36, 0.5))' }} />
                </div>
                ...
                <p style={{ animationDelay: '400ms' }} className='animate__animated animate__fadeInUp text-xs font-light text-white/50 mt-0.5'>
                    {t("vote_instruction")}
                </p>
            </div>
            ...
            <div style={{ animationDelay: '500ms' }} className='animate__animated animate__fadeInUp relative z-10 w-full max-w-lg px-4 mt-3 mb-2 flex items-center justify-center'>
                <div className={'rounded-xl py-2 px-6 flex items-center justify-center gap-2 font-bold text-sm backdrop-blur-sm shadow-lg ' + (myRoom === 1 ? 'border-2 border-blue-400 bg-blue-500/20 text-white' : 'border-2 border-rose-400 bg-rose-500/20 text-white')}>
                    <BsFillDoorOpenFill className='text-base' />
                    <span>{t("room")} {myRoom} {t("candidates")}</span>
                </div>
            </div>

            {/* Player Grid */}
            <div style={{ animationDelay: '700ms' }} className='animate__animated animate__fadeInUp relative z-10 w-full max-w-lg px-4 mt-1'>
            ...
            {/* Vote Status */}
            <div style={{ animationDelay: '1000ms' }} className='animate__animated animate__fadeInUp relative z-10 w-full max-w-lg px-5 mt-3'>
                {/* Vote count bar */}
                <div className='flex items-center justify-center gap-2 text-xs font-medium text-white/40 mb-2'>
                    ...
                    <span>{totalVotes} / {totalPlayerCount || players.length} {t("votes")}</span>
                    {totalVotes >= (totalPlayerCount || players.length) && (
                        <span className='text-green-400 animate-pulse ml-1'>✓ {t("all_voted") || "All voted!"}</span>
                    )}
                </div>
                ...
                {myVote && (
                    <div className='bg-white/10 backdrop-blur-md border border-white/20 rounded-xl p-2.5 flex items-center justify-between gap-2 mb-2'>
                        <div className='flex items-center gap-2'>
                            <span className='text-xs font-medium text-white/60'>{t("you_voted_for")}:</span>
                            <span className='text-sm font-bold text-amber-400'>{votedPlayerName}</span>
                        </div>
                        <button
            ...
            <div style={{ animationDelay: '1200ms' }} className='animate__animated animate__fadeInUp relative z-10 w-full max-w-lg px-5 mt-3 flex flex-col gap-2 pb-20'>
                {isHost && (
                    <>
                        <button
...
function VoteCard({ player, isMe, isVoted, isLeading, isCurrentLeader, voteCount, onClick, disabled = false, delay = 0, t }) {
    ...
    const cardStyle = useMemo(() => {
        if (isVoted) {
            return {
                background: 'linear-gradient(145deg, rgba(139, 92, 246, 0.4), rgba(168, 85, 247, 0.3))',
    ...
    return (
        <div
            ...
            className={'animate__animated animate__fadeInUp rounded-2xl p-2.5 flex flex-col items-center justify-center transition-all duration-300 relative overflow-hidden backdrop-blur-sm ' + (disabled ? ' opacity-50 cursor-default ' : ' cursor-pointer active:scale-95 ')}
            onClick={disabled ? undefined : onClick}
        >
            {isLeading && (
                <div className="absolute inset-0 rounded-2xl animate-pulse" style={{ background: 'radial-gradient(circle at center, rgba(251, 191, 36, 0.1), transparent 70%)' }} />
            )}

            {isCurrentLeader && (
                <div className='absolute top-1 right-1 z-10'>
                    <FaCrown className="text-amber-400 text-[10px]" style={{ filter: 'drop-shadow(0 1px 3px rgba(251, 191, 36, 0.5))' }} />
                </div>
            )}

            <div className={'relative rounded-full transition-all duration-300 ' + (isVoted ? ' ring-2 ring-purple-400 ring-offset-1 ring-offset-transparent ' : '')}>
                <Avatar className='rounded-full' style={{ height: "3rem", width: "3rem" }} {...avaConfig} />
                {isVoted && (
            ...
            <h3 className={'truncate w-full text-center text-[11px] mt-1.5 transition-colors duration-300 font-bold ' + (isMe ? ' text-purple-300 ' : ' text-white/70 ')}>
                {player?.name}
            </h3>

            <div className={'flex items-center justify-center gap-1 mt-0.5 text-sm font-bold transition-all duration-300 ' + (isLeading && voteCount > 0 ? ' text-amber-400 scale-110 ' : ' text-white/30 ')}>
                {voteCount > 0 ? (
