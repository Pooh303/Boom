...
export default function CardsFilter({
  ...
  filter = { visibleCards: undefined, hiddenCards: undefined },
}) {
  ...
  const sortAllCards = useCallback(() => {
    ...
    if (filter?.visibleCards)
      all = all.filter((card) => filter?.visibleCards?.includes(card?.id)); // only shows cards that are specified in prop "filter.visibleCards"
    if (filter?.hiddenCards)
      all = all.filter((card) => !filter?.hiddenCards?.includes(card?.id));
  ...
  const [pairedCards, setPairedCards] = useState(
    paired ? pairUpCards(allCards) || [] : [],
  );

  const filterVisibleCards = useCallback(
    (query) => {
      ...
      function checkText(card) {
        if (
          ...
          card?.name_th?.toLowerCase()?.includes(q) ||
          card?.description?.toLowerCase()?.includes(q) ||
          card?.description_th?.toLowerCase()?.includes(q) ||
          card?.color?.title?.toLowerCase()?.includes(q) ||
          card?.id?.toLowerCase()?.includes(q) ||
          card?.tags?.toLowerCase()?.includes(q) ||
          card?.color_name?.toLowerCase()?.includes(q)
        )
          return true;
        return false;
      }
      ...
      cards.sort((x, y) => {
        return x.name.toLowerCase().startsWith(q)
          ? -1
          : y.name.toLowerCase().startsWith(q)
            ? 1
            : 0;
      });
  ...
  const filterVisiblePairedCards = useCallback(
    (query) => {
          ...
          {row?.map((card, i) =>
            paired ? (
              <div className="flex flex-col gap-1" key={card?.[0]?.id + "" + i}>
                ...
                />
                {showDifficulty && (
                  <DifficultyPill difficulty={avgFromCards(card)} />
                )}
              </div>
            ) : (
              <div
              ...
              >
                <div
                  ...
                  className="card relative scale-[25%] md:scale-[50%] -mx-24 md:-mx-16 -my-36 md:-m-24 "
                >
  ...
  return (
    <div className="w-full flex-col flex items-center p-2  pb-96 gap-4 relative pt-14">
      <div
        className={
          "flex flex-col absolute inset-0 p-3 bottom-auto z-10 " +
          searchContainerClassName
        }
      ...
      >
        <div
          className={
            "flex flex-col border border-neutral bg-base-100 w-full rounded-lg rounded-b-xl transition-all " +
            (searchFocused ? " h-18 " : " h-8 ")
          }
        >
          <input
            type="text"
            ...
            placeholder="Search"
            className="w-full input input-sm shadow focus:outline-none scale-[101%] -mt-px "
          />
          {searchFocused && (
            <div className="w-full flex items-center p-2 gap-2 overflow-x-scroll scrollbar-hide h-full overflow-y-hidden">
              {SEARCH_PILLS.map((data, i) => (
                <SearchPill
                  key={data?.searchValue || i}
                  selected={
                    search?.toLowerCase() === data?.searchValue?.toLowerCase()
                  }
        ...
        {pairedCards?.[0]
          ? pairedCards.map((cardPair, i) => (
              ...
              >
                <LinkedCardsContainer
                  cards={cardPair}
                  onClick={() => onClick(cardPair[0])}
                />
                {showDifficulty && (
                  <DifficultyPill difficulty={avgFromCards(cardPair)} />
                )}
              </div>
            ))
...
function SearchPill({
  ...
  return (
    <div
      ...
      className={
        " whitespace-nowrap rounded-lg border border-neutral text-neutral bg-base-100 text-xs px-3 py-1 h-6 flex items-center justify-center transition-all " +
        (selected
          ? " shadow-sm shadow-neutral/50 order-first "
          : " shadow-none ")
      }
      style={style}
    >
