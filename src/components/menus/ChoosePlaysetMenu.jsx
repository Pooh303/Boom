  ...
  return (
    <div className="w-full h-fit rounded-xl text-base-content flex flex-col items-center overflow-x-hidden overflow-y-scroll bg-base-100 transition-all scrollbar-hide">
      ...
      <div className="w-full ">
        <h1 className="text-xl uppercase font-extrabold -mb-8 -mt-8 text-left p-5">
          Playsets
        </h1>
        ...
        <TabRow>
          ...
          {TABS.map((TAB, i) => (
            <Tab
              ...
              {...TAB}
              onClick={(d) => setTab(d)}
              color={playsets?.[TAB.playsetName]?.[0]?.color || TAB?.color}
          ...
          {devMode && (
            <Tab
              title="Development"
              onClick={(d) => setTab(d)}
              color={playsets?.["dev"]?.[0]?.color || TAB?.color}
              selected={tab === "Development"}
            />
          )}
        </TabRow>
      </div>
      {TABS.map((TAB, i) =>
        playsets?.[TAB.playsetName] ? (
          tab === TAB.title && (
            <PlaysetList
              onClick={onClick}
              key={i}
              playerCount={playerCount}
              playsets={playsets[TAB.playsetName]}
              currentPlayset={currentPlayset}
            />
          )
        ) : (
          <></>
      ...
      {tab === "For you" && (
        <TabPage
          className={
            "w-full flex flex-col gap-4 items-start justify-start px-5 pb-5"
          }
        >
          {forYou
            .filter((p) => p.id !== currentPlayset.id)
            .map((p) => (
              <div key={p.id} className=" w-full ">
                <PlaysetDisplay playset={p} onClick={() => onClick(p)} />
              </div>
            ))}
        </TabPage>
      )}
...
export function calculatePlaysetDisabled(playset, playerCount) {
  ...
  const min =
    playset.min_players ??
    (playset.players ? parseInt(playset.players.split("-")[0]) : 0);
  const max =
    playset.max_players ??
    (playset.players ? parseInt(playset.players.split("-")[1]) : 9999);
...
function sortPlaysets(playset, playerCount, shuffle = true) {
  ...
  pl = pl.sort((x, y) => {
    ...
    return playerCount >= xmin && playerCount <= xmax
      ? -1
      : playerCount >= ymin && playerCount <= ymax
        ? 1
        : xmin - ymin;
...
function PlaysetList({ playsets, playerCount, currentPlayset, onClick }) {
  const sortedPlaysets = useMemo(
    () =>
      sortPlaysets(
        playsets.filter((p) => p.id !== currentPlayset.id),
        playerCount,
        false,
      ),
    [playsets],
