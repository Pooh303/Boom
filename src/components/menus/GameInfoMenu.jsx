...
function GameInfoMenu({
  ...
  const [playWithColorReveal, setPlayWithColorReveal] = useState(
    game?.color_reveal,
  );
  const [remoteMode, setRemoteMode] = useState(game?.remote_mode);
  const [allowSwapping, setAllowSwapping] = useState(game?.allow_swapping);
          ...
          if (perm === "granted") {
            navigator.serviceWorker.ready.then(function (registration) {
              registration.showNotification(card?.color?.title || "error", {
                ...
                data: {
                  ...card,
                  color: { ...card?.color, icon: undefined },
                  info: undefined,
                  cardIcon,
                  circleIcon,
                },
              ...
              registration
                .getNotifications({ tag: "KaboomCard" })
                .then((notifications) => {});
            });
          }
        });
  ...
  function changeAllowSwapping() {
    if (!isHost) return;
    execute("change-allow-swapping");
    setAllowSwapping(!allowSwapping);
  }
  ...
  return (
    <div className="w-full h-full bg-base-100 overflow-hidden flex flex-col items-center">
      ...
      <h1 className="text-title font-extrabold text-2xl py-4 text-secondary shadow-2xl shadow-base-100 bg-base-200 w-full text-center ">
        {code}
      </h1>
      <div className="w-full h-full overflow-y-scroll overflow-x-hidden scrollbar-hide pt-3">
        ...
        <div className="pt-0 flex flex-col justify-start items-start w-full shrink bg-base-100">
          <h1 className="px-4 text-xl font-extrabold text-neutral">
            CARDS IN GAME
          </h1>
          {game?.buriedCard && (
            <p className="px-4 -mt-1 text-xs font-light text-neutral">
              1 card is buried.
            </p>
          )}
        ...
        {game.soberCard && (
          <div className="pt-0 flex flex-col justify-start items-start w-full shrink bg-base-100">
            <div className="flex items-center gap-0">
              <h1 className="px-4 text-xl font-extrabold text-[#554180]">
                SOBER CARD
              </h1>
              <Info tooltip="Last round: Drunk player can switch to this card and acquire its abilities" />
            </div>
        ...
        <div className=" flex mt-2 flex-col justify-start items-start w-full shrink bg-base-100">
          <h1 className="text-xl px-4 font-extrabold text-neutral uppercase">
            OPTIONS
          </h1>
          <div className=" p-4 w-full flex items-center gap-4 flex-col">
            {isHost && (
              <>
                <ToggleButton
                  full
                  recommended={game?.players?.length > 10}
                  checked={playWithColorReveal}
                  onChange={changeColorReveal}
                >
                  {t("color_reveal")}
                </ToggleButton>
                <ToggleButton
                  full
                  recommended={false}
                  hideReccomended={!remoteMode}
                  checked={remoteMode}
                  onChange={changeRemoteMode}
                  customText={t("remote_mode_desc")}
                  customTextClassName="text-secondary"
                ...
                <ToggleButton
                  ...
                  onChange={changeAllowSwapping}
                  customText={t("allow_swapping_desc")}
                  customTextClassName="text-neutral/50"
                  toggleClassName="toggle-neutral"
                >
                  {t("allow_swapping")}
                </ToggleButton>
              </>
            ...
            <button
              className="btn btn-neutral noskew w-full"
              onClick={() => {
                pushNotif();
              }}
            >
              <span className="skew pr-2 text-xl">
                <TbNotification />
              </span>{" "}
              card notification
            </button>
        ...
        {devMode && isHost && (
          <div className=" flex mt-2 flex-col justify-start items-start w-full shrink bg-base-100">
            ...
            <div className="bg-warning/30 p-4 w-full flex items-center gap-4 overflow-x-scroll scrollbar-hide">
              <button
                className="btn btn-warning noskew"
                onClick={() => {
                  endRound();
                  nextRound();
                }}
              ...
              <button
                className="btn btn-warning noskew"
                onClick={() => closeAllConn()}
              >
                close connections
              </button>
              <button
                className="btn btn-warning noskew"
                onClick={() => execute("redirect-to-lobby")}
              >
                lobby
              </button>
              <button
        ...
        <div className=" flex mt-2 flex-col justify-start items-start w-full bg-base-100 px-4">
          <h1 className="text-xl font-extrabold text-neutral uppercase">
            Controls
          </h1>
          <div
            onClick={() => setShowControls(!showControls)}
            className={
              "border-neutral border-2 text-base-content p-3 rounded-lg w-full transition-all overflow-y-scroll relative " +
              (showControls ? " h-64 " : " h-[3.3rem] ")
            }
          >
            <div className="cursor-pointer underline ">
              {showControls ? "Tap to hide" : "Show controls"}
            </div>
...
export function ToggleButton({
  ...
  return (
    <div
      ...
      className={
        "whitespace-nowrap btn-base-100 text-base-content border-2 border-neutral uppercase flex justify-between items-center h-12 px-4 text-sm font-semibold rounded-md gap-4 transition-all " +
        (full ? " w-full " : " w-fit ") +
        (disabled
    ...
    >
      <div className="flex flex-col justify-center">
        ...
        <p
          className={
            "text-xs text-error transition-all whitespace-break-spaces " +
            (!hideReccomended &&
            !disabled &&
            ...
            checked !== recommended
              ? " opacity-100 h-4 "
              : " opacity-0 h-0 ") +
            (customText ? customTextClassName : "")
          }
        >
          {customText || `Recommended: ${recommended ? "on" : "off"}`}
        </p>
      </div>
      <input
        readOnly
        ...
        disabled={disabled}
        className={
          "pointer-events-none toggle toggle-primary " + toggleClassName
        }
        checked={!!checked}
      />
    </div>
