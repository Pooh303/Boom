...
function SelectHostagesScreen({
  ...
  const candidates = useMemo(() => {
    return players.filter((p) => p.room === myRoom && p.id !== leaderId);
  }, [players, myRoom, leaderId]);
  ...
  function handleToggle(playerId) {
    ...
    if (
      selectedCount >= hostagesNeeded &&
      !myRoomSelections.includes(playerId)
    ) {
      return;
    }
  ...
  return (
    <div
      ...
      style={{
        background:
          "linear-gradient(145deg, #330510 0%, #2e0a15 30%, #3a0c1a 60%, #501121 100%)",
      }}
    >
      ...
      <div className="relative z-10 flex flex-col items-center pb-1 w-full">
        ...
        >
          {hasMyRoomConfirmed ? (
            ...
            <FaUserClock
              className="text-rose-400 text-3xl drop-shadow-lg"
        ...
        <p
          ...
          className="animate__animated animate__fadeInUp text-xs font-light text-white/70 mt-0.5 max-w-xs"
        >
          {statusText}
        </p>
      </div>
      ...
      >
        <div
          className={
            "rounded-xl py-2 px-6 flex items-center justify-center gap-2 font-bold text-sm backdrop-blur-sm shadow-lg border-2 " +
            (canConfirm
        ...
        >
          <span>
            {selectedCount} / {hostagesNeeded} {t("selected") || "Selected"}
          </span>
        </div>
      </div>
      ...
      >
            ...
            return (
              <HostageCard
                ...
                disabled={
                  !isLeader ||
                  hasMyRoomConfirmed ||
                  (!isSelected && selectedCount >= hostagesNeeded)
                }
                isLeader={isLeader}
              />
            );
        ...
        {candidates.length === 0 && (
          <div className="text-white/30 text-sm py-8">
            {t("no_players_available") || "No other players in room."}
          </div>
        )}
      </div>
      ...
      {isLeader && !hasMyRoomConfirmed && (
        <div
          ...
          className="animate__animated animate__fadeInUp relative z-10 w-full max-w-lg px-5 mt-6 flex gap-2 pb-20 justify-center"
        >
          <button
            onClick={handleConfirm}
            disabled={!canConfirm}
            className={
              "btn w-full max-w-xs border-0 text-white font-extrabold text-base h-12 rounded-xl shadow-xl transition-all " +
              (canConfirm
          ...
          >
            {canConfirm
              ? t("confirm_hostages") || "Confirm Hostages"
              : t("select_more") || "Select more..."}
          </button>
        </div>
      )}
      ...
      {(!isLeader || hasMyRoomConfirmed) && (
        <div
          style={{ animationDelay: "800ms" }}
          className="animate__animated animate__fadeInUp relative z-10 w-full max-w-lg px-5 mt-8 flex flex-col items-center pb-20"
        >
...
function HostageCard({
  ...
  const cardStyle = useMemo(() => {
    if (isSelected) {
      return {
        background:
          "linear-gradient(145deg, rgba(244, 63, 94, 0.4), rgba(225, 29, 72, 0.3))",
    ...
    >
      <div
        className={
          ...
          (isSelected
            ? " ring-2 ring-rose-400 ring-offset-1 ring-offset-transparent "
            : "")
        }
      >
        <Avatar
      ...
      <h3
        className={
          "truncate w-full text-center text-[11px] mt-2 transition-colors duration-300 font-bold " +
          (isSelected ? " text-rose-200 " : " text-white/50 ")
        }
      >
        {player?.name}
      </h3>
      ...
      {isSelected ? (
        <div className="text-[9px] text-rose-300 font-semibold mt-0.5 animate-pulse uppercase tracking-wider">
          Hostage
        </div>
      ) : (
