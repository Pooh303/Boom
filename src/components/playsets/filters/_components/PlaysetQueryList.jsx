...
function PlaysetQueryList(props) {
  ...
  const [activeToggles, setActiveToggles] = useLocalStorage(
    `active-toggles-object`,
    {
      verified: false,
      hidden: false,
      official: false,
  ...
  const activeTogglesArray = useMemo(
    () =>
...
left join playsets_metadata pm on p.id = pm.id
  ...
  const { data, fetchNextPage, hasNextPage, isLoading, isError, isFetching } =
    useInfiniteQuery({
      ...
      queryFn: queryFn,
      staleTime:
        devMode || refetchEveryTime || user?.id ? 1000 * 0 : 1000 * 60 * 5, // 5 minutes
  ...
  const { playsets, concatenatedPlaysets } = useMemo(() => {
    ...
    const maximizedPlaysets = minimizedPlaysets.map((playset) =>
      maximizePlayset(playset),
    );
  ...
  useEffect(() => {
    if (loaderRef.current) {
      ...
      var unobserve = observeElementInViewport(
        loaderRef.current,
        inHandler,
        outHandler,
        {
    ...
    <div className="w-full h-fit flex flex-col items-center justify-center gap-4">
      ...
      {logInToUse ? (
        <button
          ...
          className="btn btn-sm btn-secondary noskew"
        >
          Log in to see
        </button>
      ) : isError ? (
        <div className="w-full flex py-4 items-center justify-center text-sm text-red-600/50">
          An error occurred
        </div>
      ) : loading ? (
        ...
        <div className="w-full flex py-4 items-center justify-center text-sm text-base-300">
          No playsets found
        </div>
      ) : (
        <>
