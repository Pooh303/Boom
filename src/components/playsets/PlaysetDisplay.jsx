...
import { allCardsInRow, isPresetPlayset } from "../../helpers/playsets";
...
function PlaysetDisplay({
  ...
  const { bookmarked: _bookmarked, upvote: _upvote } = useMemo(() => {
  ...
  const votes = useMemo(() => {
    if (user_upvote !== myVote)
      return (
        upvote_count -
        downvote_count +
        (myVote === null ? 0 : myVote ? 1 : -1) -
        (user_upvote === null ? 0 : user_upvote ? 1 : -1)
      );
  ...
  const height = useMemo(() => {
    ...
    if (quickActions && !noQuickActions)
      heightArray.push(INTERACTION_ROW_BOCK_HEIGHT);

    const height = heightArray.reduce(
      (accumulator, currentValue, currentIndex) =>
        accumulator +
        currentValue +
        (currentIndex < heightArray.length - 1 ? GAP : 0),
      0,
    );
  ...
  return (
    <div
      ...
      className={
        "w-full transition-all overflow-y-hidden scrollbar-hide flex flex-col items-center justify-start " +
    ...
    >
      <TitleBlock
        ...
        {...{ forceOpen, noOpen, open, onClick, toggleOpen }}
      />

      <div
        style={{ height: open ? `${CARDS_BLOCK_HEIGHT + GAP}rem` : 0 }}
        className="w-full overflow-hidden transition-all"
      >
        {open && (
          <CardsBlock
            {...{ difficulty, cards, primaries, odd_card, default_cards }}
          />
        )}
      </div>
      {showPills && (
        <PillsBlock {...{ difficulty, verified, official, cardCounts }} />
      )}
      {quickActions && !noQuickActions && (
        <InteractionRowBlock
          {...{ id, votes, myVote, bookmarked }}
          quickActions={{
            ...quickActions,
            bookmark: isPresetPlayset(id) ? false : quickActions?.bookmark,
          }}
          onBookmarkedChange={(...arr) =>
            checkAuth(() => handleBookmark(...arr))
          }
          onVoteChange={(...arr) => checkAuth(() => handleVote(...arr), user)}
        />
      )}
    </div>
  );
...
function TitleBlock({
  ...
  return (
    <div
      className={
        "bg-black w-full relative h-14 pb-1 overflow-hidden rounded-xl transition-all grow flex-1 "
      }
    >
      <div
        style={{
          background: `linear-gradient(65deg, #00000000, ${color || "#c342ff"}63, ${color || "#c342ff"}ab)`,
        }}
        className="playset-gradient w-full absolute inset-0 opacity-50"
      />
      <div className="h-14 flex items-center absolute top-0 left-0 right-0">
        <div onClick={onClick} className="flex items-center w-full h-14 pr-4">
          <div
            className="h-full w-14 tooltip tooltip-right relative"
            data-tip={name}
          >
            ...
            <div className="absolute inset-0 w-full h-full flex items-center justify-center z-30 translate-x-3 translate-y-2 text-lg">
              {remixed_from && !hidden && (
                <BsStars color="#fad623" className=" text-shadow rotate-3" />
              )}
              {hidden && (
                <FaGhost color="#bdfcff" className=" text-shadow rotate-3" />
              )}
            </div>
          </div>
          ...
          <div
            className={
              "flex w-full shrink " +
              (forceOpen || noOpen
                ? " flex-row items-center justify-between text-lg "
                : " flex-col text-sm ")
            }
          >
            ...
            <div
              style={{ color: color ? `${color}` : "#c342ff" }}
              className={
                "text-normal font-bold text-sm items-center gap-1 flex  " +
                (forceOpen || noOpen ? " -ml-56 " : " ml-0 ")
              }
            >
              {dev ? (
                <MdConstruction color="#fbbf24" className="mr-1" />
              ) : official ? (
                <BsFillCheckSquareFill
                  color="#ffffff"
        ...
        {!forceOpen && !noOpen && (
          <div
            onClick={() => toggleOpen()}
            className="absolute top-0 right-0 bottom-0 w-14 flex items-center justify-center "
          >
            <div
              className={
...
function PillsBlock({ difficulty, verified, official, cardCounts }) {
  return (
    <div className="flex items-center gap-2 py-1 w-full overflow-x-scroll overflow-y-hidden scrollbar-hide">
      {difficulty && <DifficultyPill difficulty={difficulty} />}
      {verified && <VerifiedPill />}
      {official && <OfficialPill />}
      {cardCounts?.map((c, i) => (
        <Pill Icon={<TbCards />} key={i} textColor={c.color}>
          {c?.count}
        </Pill>
      ))}
    </div>
...
function InteractionRowBlock({
  ...
  return (
    <div className="flex items-center gap-2 px-4 justify-between w-full h-8 text-lg sm:text-base -translate-y-1">
      {quickActions?.vote && (
        <VoteComponent upvote={myVote} onChange={onVoteChange} count={votes} />
      )}
      {quickActions?.workbench && (
        <FaTools
          onClick={() => smoothNavigate(`/workbench/${id}`)}
          className="clickable hover:scale-100 scale-[.80] hover:text-secondary hover:rotate-365"
          title="Workbench"
        />
      )}
      {quickActions?.open && (
        <a target="_blank" href={`/playsets/${id}`}>
          <FiExternalLink
            className="clickable hover:scale-105 hover:text-purple-600 "
            title="Open"
          />
        </a>
      )}

      {quickActions?.bookmark && (
        <BookmarkComponent
...
function DifficultyInPlayset({ difficulty }) {
  const difficultyData = useMemo(
    () => getDifficultyDataFromValue(difficulty),
    [difficulty],
  );
  ...
  return (
    <div className="px-2  bg-grey-100 flex flex-col items-center justify-center gap-2">
      <LargeRadialProgress
        color={difficultyData?.colors?.primary}
        text={difficultyData?.difficulty}
        value={difficultyData?.difficulty * 10}
      />
      <Pill
        bgColor={difficultyData?.colors?.secondary}
        textColor={difficultyData?.colors?.primary}
      >
        {difficultyData?.name}
      </Pill>
    </div>
...
function PlaysetDisplayOld({
  onClick,
  playset,
  disabled = false,
  selected,
  showDisabledError = "bottom",
}) {
  const { setMenu } = useContext(PageContext);

  return (
    <>
      {showDisabledError && showDisabledError === "top" && disabled && (
        <WrongPlayerNumberPlayset />
      )}
      <div
        className={
          "bg-base-200 flex flex-col w-full h-full overflow-hidden " +
          (onClick ? " clickable " : "") +
          (disabled ? " opacity-40 " : " opacity-100 ") +
          (selected
            ? " rounded-lg border-2 border-secondary scale-100 "
            : selected === false
              ? " border-0 rounded-lg "
              : " border-0 rounded-lg  ")
        }
      >
        ...
        <div className="overflow-x-scroll flex gap-6 px-5 py-3 pb-4 overflow-y-hidden">
          <CardsRow
            cards={playset?.cards.filter((c) => !c.id.endsWith("000"))}
          />
          {playset?.odd_card && (
            <div
              ...
              onClick={() =>
                setMenu2(
                  <CardInfoMenu
                    ...
                    color={playset.odd_card.color}
                    onCancel={() => setMenu2(null)}
                  />,
                )
              }
              className="card relative scale-[30%] -m-24 -my-36"
            >
              <CardFront
                card={playset.odd_card}
                color={playset.odd_card?.color}
      ...
      {showDisabledError && showDisabledError === "bottom" && disabled && (
        <WrongPlayerNumberPlayset />
      )}
    </>
  );
...
export function CardsRow({ cards }) {
  const { setMenu2 } = useContext(PageContext);
  ...
  return cards.map((card, i) =>
    card ? (
      <div
        ...
        className="card relative scale-[20%] -m-28 -my-40"
      >
...
export function PlayWithBuryToggle({
  bury,
  recommendBury,
  onChange = () => true,
  disabled,
...
function BookmarkComponent({ bookmarked, onChange }) {
  return bookmarked ? (
    <IoBookmark
      className="text-info hover:scale-105"
      onClick={() => {
        onChange(!bookmarked);
      }}
    />
